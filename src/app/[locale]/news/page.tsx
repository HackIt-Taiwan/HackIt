...
export default function NewsPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState(t("newsPage.categoryAll") as string);
  ...
  const filteredNews = newsData.filter(news => {
    ...
    const matchesCategory = selectedCategory === t("newsPage.categoryAll") as string || news.frontmatter.category === getCategoryTranslation(selectedCategory, false);
    return matchesSearch && matchesCategory;
  });
  
  // 獲取所有類別並翻譯
  const allCategories = ["全部", "活動公告", "活動預告", "合作消息", "媒體報導"];
  const categories = [
    t("newsPage.categoryAll") as string,
    ...Array.from(new Set(newsData.map(news => getCategoryTranslation(news.frontmatter.category, true))))
  ];
  
  // 類別翻譯對應
  function getCategoryTranslation(category: string, toCurrentLocale: boolean) {
    const categoryMap: { [key: string]: string } = {
      "活動公告": "eventAnnouncement",
      "活動預告": "eventPreview",
      "合作消息": "partnerNews",
      "媒體報導": "mediaReport",
      "eventAnnouncement": "活動公告",
      "eventPreview": "活動預告",
      "partnerNews": "合作消息",
      "mediaReport": "媒體報導"
    };
    ...
    if (toCurrentLocale) {
      ...
      // 從英文轉回中文類別名稱
      for (const [zhCategory, key] of Object.entries(categoryMap)) {
        if (t(`newsPage.categories.${key}`) === category) {
          return zhCategory;
        }
      }
      return category;
  ...
  const getCategoryIcon = (category: string) => {
    const originalCategory = getCategoryTranslation(category, false);
        ...
        <div className="container mx-auto px-4">
          ...
          >
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 text-gray-800 dark:text-white">
              <span dangerouslySetInnerHTML={{ __html: t("newsPage.title") as string }}></span>
            </h1>
            ...
            <div className="max-w-2xl mx-auto mt-8 relative">
              <div className="relative">
                <input
                  type="text"
                  placeholder={t("newsPage.searchPlaceholder") as string}
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-center flex-wrap gap-2">
            {categories.map((category, index) => (
              ...
              >
                {category === t("newsPage.categoryAll") ? t("newsPage.categoryAll") as string : (
        ...
        <div className="container mx-auto px-4">
          {filteredNews.length > 0 ? (
                    ...
                    <div className="relative h-48 overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3">
                        <span className="px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300">
                          ...
                          {getCategoryTranslation(news.frontmatter.category, true)}
                        </span>
                      </div>
                    </div>
                  </Link>
                  <div className="p-5">
                    <div className="flex items-center justify-between mb-3 text-xs text-gray-500 dark:text-gray-400">
                      <div className="flex items-center">
                        ...
                        <span>{new Date(news.frontmatter.date).toLocaleDateString(locale === 'en' ? 'en-US' : 'zh-TW', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                      </div>
                      <div className="flex items-center">
                    ...
                    >
                      {t("newsPage.readMore")} <FaChevronRight className="ml-1 text-xs" />
                    </Link>
                  </div>
                </motion.article>
              ))}
            ...
            <div className="text-center py-20">
              <h3 className="text-2xl font-bold text-gray-500 dark:text-gray-400 mb-4">{t("newsPage.noResults.title")}</h3>
              <p className="text-gray-500 dark:text-gray-400">{t("newsPage.noResults.subtitle")}</p>
              <button 
                className="mt-6 px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-colors"
                onClick={() => {
                  setSearchQuery("");
                  setSelectedCategory(t("newsPage.categoryAll") as string);
                }}
              >
                {t("newsPage.noResults.clearButton")}
              </button>
            </div>
          )}
        </div>
          ...
          <div className="max-w-3xl mx-auto text-center">
            ...
            >
              ...
              <p className="text-lg text-gray-600 dark:text-gray-300 mb-8">
                {t("newsPage.subscribe.subtitle")}
              </p>
              <form className="max-w-lg mx-auto">
                <div className="flex flex-col sm:flex-row gap-3">
                  <input 
                    type="email" 
                    placeholder={t("newsPage.subscribe.emailPlaceholder") as string} 
                  ...
                  >
                    {t("newsPage.subscribe.button") as string}
                  </button>
                </div>
              </form>
            </motion.div>
